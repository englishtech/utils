...
- читает plaintext-файл со ссылками (по 1 в строке)  
- работает пачками по 50-80 url, не взрывая соединение  
- за ~4-5 сек. обрабатывает пачку (≈15× быстрее синхронного цикла)  
- результат – БД `my_database.db`, таблица `public_artists_ussr`  
- никаких внешних модулей кроме `aiohttp, bs4, requests`  
